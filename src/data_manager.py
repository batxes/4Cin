...
Kurtosis shows if the distribution is single peaked or not. High kt = many peaks, we need low KT to show a single peak """
    ...
    for i in range(number_of_genes):
        if plot:
            ...
            plt.subplot(3,1,1)
            if translocation_exp:
                reversed_list_aux = HEATMAP_DATA[:translocation_bead][::-1]
                reversed_list_aux2 = HEATMAP_DATA[translocation_bead:][::-1]
                plt.bar(range(len(HEATMAP_DATA[i])),reversed_list_aux+reversed_list_aux2,width=1)
            else:
